...
def get_balance(user_id):
    ...
    if doc:
        return doc["balance"]
    balances_col.insert_one({"user_id": str(user_id), "balance": 0})
    return 0
...
async def income_loop():
    ...
    while True:
        ...
        if now.hour != last_hour:
            all_fighters = list(fighters_col.find({}))
...
class ShopMainView(discord.ui.View):
    ...
    async def roles_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            content=f"🎭 **Магазин ролей**\n💰 Баланс: **{bal} ликкеров** | 📈 Доход: **{income}/час**\n\nВыбери роль:",
...
class ConfirmBuyView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        if has_role(interaction.user, self.name):
            await interaction.response.edit_message(content="❌ У тебя уже есть эта роль!", view=BackExitView())
            return
        bal = get_balance(interaction.user.id)
        if bal < self.price:
            await interaction.response.edit_message(
                content=f"❌ Недостаточно ликкеров!\nНужно: **{self.price}** | У тебя: **{bal}**",
                view=BackExitView())
            return
        ...
        if not role:
            try:
                ...
                await interaction.response.edit_message(content="❌ Не удалось создать роль!", view=BackExitView())
                return
        try:
            await interaction.user.add_roles(role)
        except:
            await interaction.response.edit_message(content="❌ Не удалось выдать роль! Проверь иерархию.", view=BackExitView())
            return
...
async def баланс(interaction: discord.Interaction, участник: discord.Member = None):
    ...
    f_count = len(get_fighters(цель.id))
...
async def сброс(interaction: discord.Interaction):
    ...
    if "Владелец Сервера" not in роли:
        ...
        return
    balances_col.update_many({}, {"$set": {"balance": 0}})
    await interaction.followup.send("✅ Все балансы сброшены до **0 ликкеров**!")
...
async def сбросдохода(interaction: discord.Interaction):
    ...
    if "Владелец Сервера" not in роли:
        await interaction.followup.send("❌ У тебя нет прав!", ephemeral=True)
        return
    fighters_col.update_many({}, {"$set": {"fighters": []}})
    await interaction.followup.send("✅ Все бойцы сброшены у всех игроков!")

@tree.command(name="датьвсем", description="Дать всем участникам ликкеры")
@app_commands.describe(сумма="Сумма для каждого участника")
async def датьвсем(interaction: discord.Interaction, сумма: int):
    ...
    for member in interaction.guild.members:
        if not member.bot:
            ...
            set_balance(member.id, bal + сумма)
            count += 1
...
async def оир(interaction: discord.Interaction, сторона: app_commands.Choice[str], ставка: int):
    ...
    if сторона.value == результат:
        ...
        await interaction.response.send_message(
            f"🪙 Монета подброшена... Выпало **{результат}**!\n\n"
            f"🎉 Удача на твоей стороне!\n"
            f"✅ Ты выиграл **{ставка} ликкеров**\n"
            f"💰 Баланс: **{новый_баланс} ликкеров**")
    else:
        новый_баланс = bal - ставка
        set_balance(interaction.user.id, новый_баланс)
        await interaction.response.send_message(
...
async def нак(interaction: discord.Interaction, участник: discord.Member, сумма: int):
    ...
    await interaction.followup.send(
        f"✅ {участник.mention} получил **{сумма} ликкеров**\n"
...
async def пер(interaction: discord.Interaction, участник: discord.Member, сумма: int):
    ...
    if участник.id == interaction.user.id:
        await interaction.followup.send("❌ Нельзя переводить самому себе!", ephemeral=True)
        return
    if сумма <= 0:
        await interaction.followup.send("❌ Сумма должна быть больше 0!", ephemeral=True)
        return
    bal = get_balance(interaction.user.id)
    if сумма > bal:
        await interaction.followup.send(f"❌ Недостаточно ликкеров! Баланс: **{bal}**", ephemeral=True)
        return
...
async def fake_ban(interaction: discord.Interaction, участник: discord.Member):
    ...
    if not any(r in роли for r in BAN_ROLES):
        ...
        return
    await interaction.response.send_message(
        f"🔨 {участник.mention} **забанен на 67 секунд!**")
    try:
...
async def боксы(interaction: discord.Interaction, количество: int):
    ...
    if bal < цена:
        await interaction.followup.send(
            f"❌ Нужно **{цена}** ликкеров, у тебя **{bal}**", ephemeral=True)
        return
    ...
    текст += f"\n📈 Общий доход: **{новый_доход} ликкеров/час**"
...
@tree.command(name="всебойцы", description="Посмотреть всех возможных бойцов")
async def всебойцы(interaction: discord.Interaction):
    await interaction.response.defer()
    текст = "⚔️ **Все бойцы и их доход:**\n\n"
    for rarity, fighters in FIGHTERS.items():
        ...
        текст += f"{цвет} **{rarity}:**\n"
        for f in fighters:
            текст += f"  {f['emoji']} {f['name']} — +{f['income']}/час\n"
        текст += "\n"
    текст += "⬛ **Секретный:** ???"
    if len(текст) > 1900:
        текст = текст[:1900] + "..."
